...
if (isset($_GET['restaurant_id']) && is_numeric($_GET['restaurant_id'])) {
    ...
    if (isset($_GET['period']) &&
        ...
        $sql = "SELECT `email`,COUNT(*) AS count FROM `restaurant_bookatable` WHERE `restaurant_id` = ".$restaurant_id." AND `email` != '' AND MONTH(`booking_timestamp`) = '".$period."'  GROUP BY `email` HAVING count > 1";
