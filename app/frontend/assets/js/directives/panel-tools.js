    ...
    return {
        ...
        template: function (elem, attrs) {
            ...
            if (attrs.toolRefresh){
                temp += templates.refresh.replace(/{{refreshEvent}}/g, attrs.toolRefresh);}
            if (attrs.toolExpand)
...
app.directive('panelRefresh', function () {
    ...
    return {
        ...
        link: function ($scope, $element,$attr) {
            var panelRefresh = $scope.$eval($attr.panelRefresh);

		    var refreshEvent = panelRefresh || 'panel-refresh', csspinnerClass = 'csspinner', defaultSpinner = 'load1';
